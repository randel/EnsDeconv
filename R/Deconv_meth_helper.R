...
deconv_method_switch <- function(method_name, to_deconv, ref_matrix, meta_ref,pure_samples, markers, verb, 
  ...
  tme <- system.time(output <- switch(method_name,
                                      ...
                                      deconf = t(CellMix::ged(object = to_deconv, x = markers, method = "deconf", verbose = verb)@fit@H),
                                      ssFrobenius = t(CellMix::ged(object = to_deconv, x = markers, method = "ssFrobenius", verbose = verb)@fit@H),
                                      ssKL = t(CellMix::ged(object = to_deconv, x = markers, method = "ssKL", verbose = verb)@fit@H),
                                      DSA = t( CellMix::ged(object = to_deconv, x = markers, method = "DSA", verbose = verb)@fit@H),
                                      `Q Prog` = t(CellMix::ged(object = to_deconv, x = sig_matrix, method = "qprog", verbose = verb)@fit@H),
                                      `LS Fit` = t(CellMix::ged(object = to_deconv, x = sig_matrix, method = "lsfit", verbose = verb)@fit@H),
